...
Mechanism='bottleneck';% 'MFD' or 'bottleneck'
Capacity=0.8;
...
% tstar=rand(10,1)-0.5;
tstar=0;
population=generateSParctan(tstar,2.5*ones(size(tstar)),0.5*ones(size(tstar)),ones(size(tstar)),4*ones(size(tstar)));%

% Define some technical settings
settings.maxIter=20000;
settings.display='final'; %'on' 'off' 'final'
...
switch Mode
    case 'continuum'
        ...
        revisionProtocol.rate=0.02/length(departureTimes);
    case 'discrete'
